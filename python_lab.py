...
LofL_sum = sum(sum(x) for x in [[.25, .75, .1], [-1, 0], [4, 4, 4, 4]]) # use form: sum([sum(...) ... ])
...
## Task 16
first_of_tuples_list = [(x,y,z) for x in S for y in S for z in S if sum((x,y,z)) == 0 and (x,y,z) != (0,0,0)][1]

## Task 17
L1 = [2,2,4] # <-- want len(L1) != len(list(set(L1)))
L2 = [3,2,4] # <-- same len(L2) == len(list(set(L2))) but L2 != list(set(L2))

## Task 18
odd_num_list_range = {x for x in list(range(100))[1::2]}
